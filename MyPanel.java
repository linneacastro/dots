...
These methods are used, most often within the mouse click action event in the Dots.java file
...
repaint is called in the main Dots JFrame.  
...
public class MyPanel extends JPanel {
	...
	boolean turnflag; // Flag for changing turns.  If true then next players turn
	...
	// BOXES COMPLETED BEFORE TURN - If Boxes completed after click is greater than boxes completed before turn, we'll keep it on current player's turn  
	// because a box was just completed.  
	public void boxesCompletedBeforeTurn() {
		for(int j=0; j<totalRows; j++) { // Iterate through all rows
			for(int i=0; i<totalCols; i++) { // Iterate through all columns
				if(arrayOfBoxes[j][i].whoOwnsThisBox()!=' ') { // Count all owned boxes, everything that isn't empty space.
					boxesCompletedBeforeTurn++; // Increment box count
				}
			}
		}
	}
	...
	// BOXES COMPLETED AFTER TURN - if Boxes completed after click is greater than boxes completed before turn, we will keep it on current player's turn because
	// they completed a box.  
	public void boxesCompletedAfterTurn() {
		for(int j=0; j<totalRows; j++) { // Iterate through all rows
			for(int i=0; i<totalCols; i++) { // Iterate through all columns
				if(arrayOfBoxes[j][i].whoOwnsThisBox()!=' ') { // Count all owned boxes, irrespective of ownership.
					boxesCompletedAfterTurn++; // Increment box count
				}
			}
		}
	}
	...
	public void claimBox() {
		if(arrayOfBoxes[currentRow][currentCol].boxed()) {
			...
			turnflag = false;  // Box was just completed, current player will go again
		}
	}
		
		
	// CALCULATE OR UPDATE PLAYER 1 SCORE - This method calculated Player 1's score internally, by iterating
	// through all boxes and incrementing score if initials match that of Player 1.
	public void calculatePlayer1Score() { // Method has no input
		for(int j=0; j<totalRows; j++) { // Iterate through all rows
			for(int i=0; i<totalCols; i++) { // Iterate through all columns
				if(arrayOfBoxes[j][i].whoOwnsThisBox()==playerinitials[0]) { // If whoOwnsThisBox char matches P1 initial...
					player1Score++; // Increment player1Score
				}
			}
		}
	}
	...
	// main program to display player 1's cumulative score.  
		public int P1ScoreAsInt() {
			player1Score = 0;  // Scores must be reset to 0 before recounting, otherwise score will compound and not be accurate.
			for(int j=0; j<totalRows; j++) {
				for(int i=0; i<totalCols; i++) {
	...
	// CALCULATE OR UPDATE PLAYER 2 SCORE - This method calculated Player 2's score internally, by iterating
    // through all boxes and incrementing score if initials match that of Player 2.
	public void calculatePlayer2Score() { // Method has no input or specified output
		for(int j=0; j<totalRows; j++) { // Iterate through all rows and columns
			for(int i=0; i<totalCols; i++) {
				if(arrayOfBoxes[j][i].whoOwnsThisBox()==playerinitials[1]) {
					player2Score++; // Increment player2Score if box ownership initial matches Player 2's initials
				}
			}
		}
	}
	...
	public int P2ScoreAsInt() {
		player2Score = 0; // Scores must be reset to 0 before recounting, otherwise score will compound and not be accurate.
		for(int j=0; j<totalRows; j++) {
			for(int i=0; i<totalCols; i++) {
				if(arrayOfBoxes[j][i].whoOwnsThisBox()==playerinitials[1]) {
					player2Score++; 
	...
	// CALCULATE WINNER METHOD - This method is called once all boxes have been claimed.  It tallies up ownership
	// of all boxes.  The player with the most boxes claimed wins.  If number of boxes claimed is equal, a tie is 
	// called.
	...
	// RESET METHOD - The reset method is used when a user hits the reset button.  It clears all booleans and ownership 
	// initials associated with a box.  It also clears the two arrays used for holding player initials and names, which 
	// are used for calculating and aligning turns.
	void gameReset() 
	{
		for (int i=0; i < boxesInRow; i++) 
		{
			for (int j = 0; j < boxesInCol; j++) 
			{
				arrayOfBoxes[i][j].setBottom(false); // Clear all booleans and initials for all boxes in array
				arrayOfBoxes[i][j].setTop(false);   
				arrayOfBoxes[i][j].setRight(false);
				arrayOfBoxes[i][j].setLeft(false);
				arrayOfBoxes[i][j].setWhoOwnsThisBox(' ');
			}
		}
		
		for (int i =0; i < 2; i++) { // Clear array used for turns holding assignments for player names and initials
			playerinitials[i] = ' ';
			playernames[i] = "";
		}
		turn = 0; // Reset turn
